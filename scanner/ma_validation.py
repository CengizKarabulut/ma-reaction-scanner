...
def simulate_event_trades(
    ...
    for event in sorted(events, key=lambda item: item.position):
        ...
        target = entry + side * config.target_atr * atr_value
        stop = entry - side * config.stop_atr * atr_value
        estimated_round_trip_cost = entry * (
            2.0 * cost.commission_rate + cost.entry_friction + cost.exit_friction
        )
        ...
        rows.append({
            ...
            "gross_pnl": gross_pnl,
            "costs": commissions + abs(raw_entry - entry) * shares + abs(raw_exit - exit_price) * shares,
            "net_pnl": net_pnl,
